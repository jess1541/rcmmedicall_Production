...
const ExecutiveCalendar: React.FC<ExecutiveCalendarProps> = ({ doctors, onUpdateDoctor, onDeleteVisit, user }) => {
  ...
  const handleTrashDrop = (e: React.DragEvent) => {
      ...
      try {
          ...
          if (docId && visitId && window.confirm("¿Confirmas que deseas eliminar este elemento?")) {
  ...
  const handleOpenTimeOffModal = () => {
      setNewTimeOff({ startDate: new Date().toISOString().split('T')[0], endDate: new Date().toISOString().split('T')[0], duration: 'TODO EL DÍA', reason: 'JUNTA', notes: '' });
  ...
  const handleSaveTimeOff = () => {
      const event: TimeOffEvent = {
          ...
          duration: newTimeOff.duration as any,
          reason: newTimeOff.reason as any,
  ...
  const renderEventChip = (evt: any, i: number, isSlot: boolean = false) => {
      ...
      const chipClasses = isSlot 
        ? `absolute left-0 right-0 mx-2 p-2 rounded shadow-sm cursor-pointer transition-colors z-10 border-l-4 ${isAppointment ? 'bg-pink-100 border-pink-500 hover:bg-pink-200 cursor-default' : (isCompleted ? 'bg-green-100 border-green-500 hover:bg-green-200' : 'bg-blue-100 border-blue-500 hover:bg-blue-200')}`
        : `px-2 py-1 rounded text-[8px] font-bold border shadow-sm flex items-center gap-1 transition-all hover:scale-[1.02] relative pr-1 cursor-grab active:cursor-grabbing w-full mb-1 ${isTimeOff ? 'bg-gradient-to-r from-orange-50 to-orange-100 border-orange-200 text-orange-800' : (isAppointment ? 'bg-gradient-to-r from-pink-500 to-rose-500 text-white border-transparent shadow-pink-300' : (isCompleted ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-transparent shadow-emerald-300' : 'bg-gradient-to-r from-blue-500 to-indigo-600 text-white border-transparent shadow-blue-200'))}`;
      ...
      return (
        ...
        >
            {isSlot ? (
                <>
                    <span className={`text-xs font-bold block ${isAppointment ? 'text-pink-900' : (isCompleted ? 'text-green-900' : 'text-blue-900')}`}>{evt.data.docName} ({evt.data.docCategory || 'MEDICO'})</span>
                    <span className={`text-[10px] uppercase ${isAppointment ? 'text-pink-700' : (isCompleted ? 'text-green-700' : 'text-blue-700')}`}>{evt.data.visit.objective}</span>
                </>
            ) : (
                <>
                    <span className="truncate leading-tight flex-1">{!isTimeOff && <span className="font-black mr-1 opacity-90">{evt.data.visit.time || '??:??'}</span>}{isTimeOff ? evt.data.reason : evt.data.docName}</span>
                    {isCompleted && <CheckCircle2 className="w-2.5 h-2.5 text-white flex-shrink-0" />}{isAppointment && <Lock className="w-2 h-2 text-white/80 ml-1" />}
                </>
            )}
        </div>
      );
    ...
    <div className="space-y-6 pb-10 relative">
       ...
       <div className="flex flex-col xl:flex-row justify-between items-center bg-white/80 backdrop-blur-xl p-4 rounded-3xl border border-white/50 shadow-lg shadow-blue-500/5 gap-4">
           ...
           <div className="flex flex-col md:flex-row gap-3 items-center w-full xl:w-auto">
               <button onClick={() => handleDayClick(currentDate, false)} className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-xl text-xs md:text-sm font-bold shadow-lg shadow-blue-500/30 transition-all active:scale-95 flex items-center w-full md:w-auto justify-center"><Calendar className="w-4 h-4 mr-2" />Programar Visita</button>
               <button onClick={() => handleDayClick(currentDate, true)} className="bg-gradient-to-r from-pink-600 to-rose-600 text-white px-4 py-2 rounded-xl text-xs md:text-sm font-bold shadow-lg shadow-pink-500/30 transition-all active:scale-95 flex items-center w-full md:w-auto justify-center"><Clock className="w-4 h-4 mr-2" />Programar Cita</button>
               <button onClick={handleOpenTimeOffModal} className="bg-gradient-to-r from-orange-400 to-orange-600 text-white px-4 py-2 rounded-xl text-xs md:text-sm font-bold shadow-lg shadow-orange-500/20 transition-all active:scale-95 flex items-center w-full md:w-auto justify-center"><Coffee className="w-4 h-4 mr-2" />Ausencia</button>
               ...
               {user.role === 'admin' && (
                   <div className="flex items-center bg-blue-50/50 p-1.5 pl-3 rounded-xl border border-blue-100/50 w-full md:w-auto">
                       ...
                       <select className="bg-transparent border-0 text-blue-900 text-sm font-bold focus:ring-0 block p-1 cursor-pointer w-full" value={selectedExecutive} onChange={(e) => setSelectedExecutive(e.target.value)}>{executives.map(e => <option key={e} value={e}>{e}</option>)}</select>
                   </div>
               )}
           </div>
       </div>
               ...
               <div className={`h-full flex flex-col ${viewMode !== 'day' ? 'min-w-[800px]' : 'min-w-full'}`}>
                   ...
                   <div className={`flex-1 ${viewMode === 'day' ? 'flex flex-col' : 'grid grid-cols-7 auto-rows-fr gap-px'}`}>
                       {calendarDays.map((day, idx) => {
                                       ...
                                       <div className="w-24 flex-shrink-0 border-r border-slate-100 pr-4 pt-2">
                                           {visitTimeSlots.map(time => (
                                               <div key={time} className="h-24 text-xs font-bold text-slate-400 flex items-start justify-between group relative">
                                                   ...
                                                   <button onClick={(e) => { e.stopPropagation(); handleTimeSlotClick(time); }} className="absolute right-0 top-0 opacity-0 group-hover:opacity-100 bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white rounded-full p-1 transition-all transform hover:scale-110 z-20" title="Planear Visita aquí"><Plus className="h-3 w-3" /></button>
                                               </div>
                                           ))}
                                       </div>
                                       <div className="flex-1 pl-4 relative pt-2">
                                            {visitTimeSlots.map((time, tIdx) => (
                                                 <div key={`line-${time}`} className="absolute w-full border-t border-slate-100" style={{ top: `${tIdx * 6}rem` }}></div>
                                            ))}
                                            {visitTimeSlots.map((time, tIdx) => {
                                                const slotEvents = events.filter(e => e.type === 'visit' && e.data.visit.time === time);
                                                return (
                                                    <div key={time} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, day, time)} className="absolute w-full hover:bg-blue-50/30 transition-colors z-0" style={{ top: `${tIdx * 6}rem`, height: '6rem' }}>
                           ...
                           return (
                               <div key={idx} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, day)} onClick={() => handleDayClick(day)} className={`min-h-[120px] bg-white p-2 border border-slate-100 hover:bg-blue-50/20 transition-colors cursor-pointer relative group flex flex-col gap-1 ${isToday ? 'ring-1 ring-inset ring-blue-200 bg-blue-50/10' : ''}`}>
                                   <div className="flex justify-between items-start">
                                        <span className={`text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full ${isToday ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500'}`}>{day.getDate()}</span>
                                        <button className="opacity-0 group-hover:opacity-100 bg-slate-100 p-1 rounded-full text-slate-400 hover:text-blue-500 transition-all"><Plus className="h-3 w-3" /></button>
                                   </div>
                   ...
                   <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                       <div>
                           <div className="flex space-x-4 mb-2">
                                <button onClick={() => setIsEditingPlan(false)} className={`text-sm font-black uppercase border-b-2 pb-1 transition-colors ${!isEditingPlan ? 'text-blue-600 border-blue-600' : 'text-slate-400 border-transparent hover:text-slate-600'}`}>Reportar</button>
                                <button onClick={() => setIsEditingPlan(true)} className={`text-sm font-black uppercase border-b-2 pb-1 transition-colors ${isEditingPlan ? 'text-blue-600 border-blue-600' : 'text-slate-400 border-transparent hover:text-slate-600'}`}>Editar Plan</button>
                           </div>
                           <h3 className="text-base font-bold text-slate-800 uppercase flex items-center">{isEditingPlan ? 'Modificar Planificación' : 'Reportar Resultado'}</h3>
                       </div>
                       <div className="flex items-center gap-2">
                            <button onClick={confirmDeleteVisit} className="p-2 text-red-400 hover:bg-red-50 hover:text-red-600 rounded-xl transition-colors"><Trash2 className="w-5 h-5" /></button>
                            <button onClick={() => setReportModalOpen(false)} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors"><X className="w-6 h-6" /></button>
                       </div>
                   </div>
                   <div className="p-6 space-y-5 overflow-y-auto">
                       <div className="grid grid-cols-2 gap-4">
                           <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Fecha</label><input type="date" value={reportDate} onChange={(e) => setReportDate(e.target.value)} className="w-full border border-slate-200 rounded-xl p-2.5 text-sm font-bold bg-slate-50" /></div>
                           <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Hora</label><select value={reportTime} onChange={(e) => setReportTime(e.target.value)} className="w-full border border-slate-200 rounded-xl p-2.5 text-sm font-bold bg-slate-50">{visitTimeSlots.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
                       </div>
                       {isEditingPlan ? (
                           <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Objetivo (Editar)</label><textarea rows={3} value={editObjective} onChange={(e) => setEditObjective(e.target.value.toUpperCase())} className="w-full border border-slate-200 rounded-xl p-3 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none uppercase" /></div>
                       ) : (
                           <>
                               <div className="bg-slate-50 p-3 rounded-xl border border-slate-100"><span className="text-xs font-black text-slate-400 uppercase block mb-1">Objetivo Original</span><p className="text-sm font-bold text-slate-700 uppercase">{editObjective}</p></div>
                               <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Resultado</label><select value={reportOutcome} onChange={(e) => setReportOutcome(e.target.value)} className="w-full border border-slate-200 rounded-xl p-3 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none bg-white"><option value="SEGUIMIENTO">SEGUIMIENTO</option><option value="COTIZACIÓN">COTIZACIÓN</option><option value="INTERESADO">INTERESADO</option><option value="PROGRAMAR PROCEDIMIENTO">PROGRAMAR PROCEDIMIENTO</option><option value="AUSENTE">AUSENTE</option></select></div>
                               <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Reporte / Notas</label><textarea rows={3} value={reportNote} onChange={(e) => setReportNote(e.target.value.toUpperCase())} className="w-full border border-slate-200 rounded-xl p-3 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none uppercase resize-none" placeholder="DETALLES DE LA VISITA..." /></div>
                               <div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Siguiente Paso</label><textarea rows={2} value={reportFollowUp} onChange={(e) => setReportFollowUp(e.target.value.toUpperCase())} className="w-full border border-slate-200 rounded-xl p-3 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none uppercase resize-none" placeholder="COMPROMISOS..." /></div>
                               <div className="border-t border-slate-100 pt-4"><div className="flex items-center mb-2"><input type="checkbox" id="scheduleNext" checked={!!nextVisitDate} onChange={(e) => { if(e.target.checked) setNextVisitDate(new Date()); else setNextVisitDate(null); }} className="w-4 h-4 text-blue-600 rounded focus:ring-blue-500 mr-2" /><label htmlFor="scheduleNext" className="text-sm font-bold text-slate-700">Agendar Próxima Visita</label></div>
                                   {nextVisitDate && (<div className="grid grid-cols-2 gap-4 mt-2 bg-blue-50 p-3 rounded-xl"><div><label className="block text-[10px] font-black text-blue-400 uppercase mb-1">Fecha</label><DatePicker selected={nextVisitDate} onChange={(date) => setNextVisitDate(date)} dateFormat="dd/MM/yyyy" locale="es" className="w-full text-xs font-bold p-2 rounded border border-blue-200" /></div><div><label className="block text-[10px] font-black text-blue-400 uppercase mb-1">Hora</label><select value={nextVisitTime} onChange={(e) => setNextVisitTime(e.target.value)} className="w-full text-xs font-bold p-2 rounded border border-blue-200">{visitTimeSlots.map(t => <option key={t} value={t}>{t}</option>)}</select></div></div>)}
                               </div>
                           </>
                       )}
                   </div>
       ...
       {isTimeOffModalOpen && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col"><div className="p-6 border-b border-slate-100 flex justify-between items-center bg-orange-50"><h3 className="text-lg font-black text-orange-800 flex items-center"><Coffee className="w-5 h-5 mr-2" />Registrar Ausencia</h3><button onClick={() => setIsTimeOffModalOpen(false)}><X className="w-6 h-6 text-orange-400 hover:text-orange-600" /></button></div><div className="p-6 space-y-4"><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Desde</label><input type="date" value={newTimeOff.startDate} onChange={(e) => setNewTimeOff({...newTimeOff, startDate: e.target.value})} className="w-full border border-slate-200 rounded-xl p-2 text-sm font-bold" /></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-1">Hasta</label><input type="date" value={newTimeOff.endDate} onChange={(e) => setNewTimeOff({...newTimeOff, endDate: e.target.value})} className="w-full border border-slate-200 rounded-xl p-2 text-sm font-bold" /></div></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Duración</label><select value={newTimeOff.duration} onChange={(e) => setNewTimeOff({...newTimeOff, duration: e.target.value as any})} className="w-full border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none"><option value="TODO EL DÍA">TODO EL DÍA</option><option value="2 A 4 HRS">2 A 4 HRS</option><option value="6 A 8 HRS">6 A 8 HRS</option></select></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Motivo</label><select value={newTimeOff.reason} onChange={(e) => setNewTimeOff({...newTimeOff, reason: e.target.value as any})} className="w-full border border-slate-200 rounded-xl p-3 text-sm font-bold outline-none"><option value="JUNTA">JUNTA</option><option value="CAPACITACIÓN">CAPACITACIÓN</option><option value="PERMISO">PERMISO</option><option value="ADMINISTRATIVO">ADMINISTRATIVO</option></select></div><div><label className="block text-xs font-bold text-slate-500 uppercase mb-2">Notas</label><textarea rows={2} value={newTimeOff.notes} onChange={(e) => setNewTimeOff({...newTimeOff, notes: e.target.value.toUpperCase()})} className="w-full border border-slate-200 rounded-xl p-3 text-sm uppercase outline-none resize-none" /></div></div><div className="p-6 border-t border-slate-100 flex justify-end gap-3 bg-slate-50"><button onClick={() => setIsTimeOffModalOpen(false)} className="px-4 py-2 text-sm font-bold text-slate-500 hover:bg-slate-200 rounded-xl transition-colors">Cancelar</button><button onClick={handleSaveTimeOff} className="px-6 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-sm font-bold shadow-lg transition-all active:scale-95">Guardar Ausencia</button></div></div></div>)}
       {selectedTimeOff && (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"><div className="bg-white rounded-3xl shadow-xl p-6 w-full max-w-sm"><div className="flex justify-between items-start mb-4"><h3 className="text-lg font-black text-slate-800 uppercase">{selectedTimeOff.reason}</h3><button onClick={() => setSelectedTimeOff(null)}><X className="w-5 h-5 text-slate-400" /></button></div><div className="space-y-2 mb-6 text-sm text-slate-600"><p><span className="font-bold">Periodo:</span> {selectedTimeOff.startDate} - {selectedTimeOff.endDate}</p><p><span className="font-bold">Duración:</span> {selectedTimeOff.duration}</p>{selectedTimeOff.notes && <p className="italic bg-slate-50 p-2 rounded border border-slate-100">"{selectedTimeOff.notes}"</p>}</div><div className="flex justify-end"><button onClick={() => handleDeleteTimeOff(selectedTimeOff.id)} className="flex items-center text-red-500 hover:text-red-700 font-bold text-sm px-4 py-2 hover:bg-red-50 rounded-xl transition-colors"><Trash2 className="w-4 h-4 mr-2" /> Eliminar Evento</button></div></div></div>)}
    </div>
  );
};
